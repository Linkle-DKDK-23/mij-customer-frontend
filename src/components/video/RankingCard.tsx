  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow ${className}`}>
      <div className="relative">
        ...
        {post.rank && (
          <div className={`absolute top-2 left-2 ${getRankBadgeColor(post.rank)} text-white text-sm font-bold w-8 h-8 rounded-full flex items-center justify-center`}>
            {post.rank}
          </div>
        )}
        <div className="absolute top-2 right-2">
          {post.rank && getRankIcon(post.rank)}
        </div>
